...
if __name__ == '__main__':
    ...
    net = Transformer(vocab_size=len(vocab), embedding_dim=embed_size,
                      hidden_dim=embed_size, num_class=labels, max_len=max_len)
    net.to(device)
    ...
    test_set = TransformerDataset([(review, 0) for review in test_reviews])
    ...
    with torch.no_grad():
        with tqdm(total=len(test_iter), desc='Prediction') as pbar:
            for test_feature, test_length, _ in test_iter:
