...
class Capsule(nn.Module):
    def __init__(self, num_hiddens, bidirectional, num_capsule=5, dim_capsule=5, routings=4, **kwargs):
        ...
        if self.bidirectional:
            self.W = nn.Parameter(
                nn.init.xavier_normal_(torch.empty(1, self.num_hiddens * 2,
                                                   self.num_capsule * self.dim_capsule)))
        else:
            self.W = nn.Parameter(
                nn.init.xavier_normal_(torch.empty(1, self.num_hiddens,
                                                   self.num_capsule * self.dim_capsule)))
...
class SentimentNet(nn.Module):
    ...
    def forward(self, inputs):
        ...
        capsule = self.capsule(states.permute(1, 0, 2).contiguous())
