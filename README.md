...
| 本地排名 | 模型 | 最佳验证准确率 | 最佳 Epoch | 关键训练设置 | 预测文件 |
|---:|---|---:|---:|---|---|
| 1 | DeBERTa-v3-large | **96.72%** | 3 | LR `1e-5`，Batch 4，梯度累积 4 | `results/deberta_v3_large_trainer.csv` |
| 2 | RoBERTa-large | **96.36%** | 2 | LR `1e-5`，Batch 16 | `results/roberta_large_trainer.csv` |
| 3 | DeBERTa-v3-base | **95.20%** | 3 | 默认 LR `5e-5`，Batch 16 | `results/deberta_v3_base_trainer.csv` |
| 4 | RoBERTa-base | **94.60%** | 3 | 默认 LR `5e-5`，Batch 16 | `results/roberta_base_trainer.csv` |
| 5 | BERT-large | **93.86%** | 3 | 默认 LR `5e-5`，Batch 16 | `results/bert_large_trainer.csv` |
| 6 | BERT-base | **93.50%** | 3 | 默认 LR `5e-5`，Batch 16 | `results/bert_base_trainer.csv` |
...
6. 在相同数据划分下，三组 large 模型均优于对应 base；DeBERTa-v3-large 的本地最佳验证准确率最高，为 96.72%，但 large 模型对显存和学习率也更敏感。
